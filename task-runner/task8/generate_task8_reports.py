#!/usr/bin/env python3
"""从 Task 8 原始结果生成公共任务规定的两份 V8 中文报告。"""
...
from run_task8_evaluation import PROJECT_ROOT, RESULT_FILE_NAME, task_identity


DEFAULT_REPORTS_DIR = PROJECT_ROOT / "longAudioTestV8" / "评测结果"
CER_REPORT_NAME = "SenseVoice_CER&Whisper-large-v3-turbo_CER_V8评价报告.md"
AUTOMATED_REPORT_NAME = "音频交付与文本一致性_V8自动检查报告.md"


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--results-dir", type=Path, required=True, help="Task 8 本次原始结果目录")
...
def load_results(results_dir: Path) -> dict[str, Any]:
    path = results_dir.expanduser().resolve() / RESULT_FILE_NAME
    try:
        results = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError(f"无法读取 Task 8 原始结果：{path}: {exc}") from exc
...
def validate_complete_results(results: dict[str, Any]) -> None:
    ...
    task_label, version = task_identity(contract)
    if results.get("schema_version") != contract["schema_version"] or results.get("version") != version:
        raise ValueError(f"结果不是 {task_label} {version} 格式")
    expected = [item["model_id"] for item in results.get("candidate_models", [])]
    if not expected:
        raise ValueError("结果缺少首次运行时冻结的候选音频清单")
...
def ranked_models(results: dict[str, Any], metric: str) -> list[dict[str, Any]]:
    eligible = [
        ...
        if record["metrics"][metric]["asr_health"]["ranking_eligible"]
    ]
    return sorted(eligible, key=lambda item: float(item["metrics"][metric]["strict_character_cer"]))


def rank_label(record: dict[str, Any], places: dict[str, int], metric: str) -> str:
    if not record["metrics"][metric]["asr_health"]["ranking_eligible"]:
        return "不参与"
    return str(places[record["model_id"]])
...
def build_cer_report(results: dict[str, Any], results_link: str) -> str:
    ...
    source_description = str(source["description"]).replace("ai_deal.json", "`ai_deal.json`")
    ...
    lines = [
        f"# SenseVoice CER 与 Whisper-large-v3-turbo CER {version} 评价报告",
        "",
        "本报告只衡量全文台词保真。唯一参考严格为 "
        f"`{source['text_path']}` 中实际参与合成、原始顺序固定的文本。{source_description}",
        "",
        ...
        f"- 评测单元：每条完整 `audio_*.wav` 成品。{task_label} 使用与最终 WAV 哈希绑定的逐段合成证据；ASR 按语义段独立转写后汇总全文 CER，不使用固定时间窗口。",
        f"- 原始证据：[{RESULT_FILE_NAME}]({results_link}/{RESULT_FILE_NAME})。",
        ...
        lines.append(
            ...
            + " | ".join(
                (
                    ...
                    format_number(float(sense["strict_character_cer"])),
                    rank_label(record, sense_places, "sensevoice_cer"),
                    format_number(float(whisper["strict_character_cer"])),
                    rank_label(record, whisper_places, "whisper_large_v3_turbo_cer"),
                    format_number(float(whisper["strict_character_cer"]) - float(sense["strict_character_cer"])),
                )
            )
            + " |"
        )
        ...
        for metric, label in backend_labels:
            ...
            lines.extend(
                [
                    ...
                    f"- 带声调拼音 CER：`{format_number(float(result['phonetic_cer']))}`；拼音 token 编辑数：`{result['phonetic_errors']}`。该指标只提示同音字假阳性可能性，不能取代严格 CER。",
            ...
            raw_transcription = "".join(
                str(chunk.get("raw_transcription", "")) for chunk in result.get("chunks", [])
            )
    ...
    for record in model_records.values():
        ...
        consensus = record["asr_consensus_health"]
        lines.extend(
            [
                ...
                f"- 同段转写共识健康：`{consensus['status']}`；分歧过大的片段：{', '.join(consensus['unreliable_segment_ids']) or '无'}。",
                "",
            ]
        )
    return "\n".join(lines).rstrip() + "\n"
...
def format_silence_regions(regions: list[dict[str, float]]) -> str:
    if not regions:
        return "无"
    return "；".join(f"{region['start_seconds']:.3f}s–{region['end_seconds']:.3f}s" for region in regions)


def build_automated_report(results: dict[str, Any], results_link: str) -> str:
    contract = results["contract"]
    _task_label, version = task_identity(contract)
    source = contract["source"]
    measurement = contract["audio_measurement"]
    lines = [
        f"# 音频交付与文本一致性 {version} 自动检查报告",
        "",
        ...
        f"- 评测单元：`{results['inputs']['evaluation_unit']}`。{results['inputs']['evaluation_unit_note']}",
        "",
        "## 音频交付原始测量",
        "",
    ...
    for record in results["models"].values():
        lines.append(f"- {record['display_name']}：{format_silence_regions(record['audio_delivery']['long_silence_regions'])}。")
    lines.extend(
        [
            "",
            "## 台词与结构完整性",
    ...
    lines.extend(
        [
            ...
            "## 文本时间与读法合规",
            "",
            contract["not_executed"]["forced_alignment"],
            "",
            "## 角色路由告警（非音色评分）",
            "",
            contract["not_executed"]["role_routing"],
        ]
    )
    return "\n".join(lines).rstrip() + "\n"
...
def main() -> int:
    ...
    try:
        ...
        print(f"Task 8 报告生成失败：{error}", file=sys.stderr)
        return 2
    print(cer_path)
