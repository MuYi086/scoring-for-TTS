...
from run_task7_evaluation import DEFAULT_CONTRACT, check_preflight, discover_models, load_contract
...
def main() -> int:
    ...
    try:
        ...
        print(f"Task 7 评测预检失败：{error}", file=sys.stderr)
        return 2
    if errors:
        print("Task 7 评测预检失败：", file=sys.stderr)
        for error in errors:
            print(f"- {error}", file=sys.stderr)
        return 2
    print(
        "Task 7 评测预检通过：text.md、共享分段清单、旁白参考音频、两条候选成品及其哈希绑定逐段证据、"
