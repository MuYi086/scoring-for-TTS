#!/usr/bin/env python3
"""Task 3 V2：直接调用 MiMo 官方云端 voiceclone 接口。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="MiMo-V2.5-TTS V2 三角色克隆")
...
def main() -> int:
    ...
    try:
        ...
        if not args.overwrite:
            existing = [
                str(output_path(case.character))
                for case in cases
                if output_path(case.character).exists()
            ]
            if existing:
