...
测试使用仓库内的三段参考音频、固定台词和校正参考文本。支持参考文本的模型会直接使用它；这避免了 OmniVoice 在离线模式下自动下载 Whisper（语音识别模型），也通常能提高克隆相似度。
...
当前 V2 脚本名称统一带 `_v2`，使用 `testData/mimo_<旁白、辰南、小公主>_v2.wav`。目标文本和三组 IndexTTS2 情感向量冻结在 `_clone_test_support_v2.py` 与 `indextts2/test_clone_indextts2_v2.py`；所有输出写入 `cloneData/audio_v2/`，命名为 `<模型名>_<人物>.wav`。

七个本地入口中，通用模型使用相应的 `run_clone_*_v2.sh`；IndexTTS2 也使用该命名。示例：

```bash
bash cloneData/qwen3_tts_12hz_1_7b_base/run_clone_qwen3_tts_12hz_1_7b_base_v2.sh
bash cloneData/indextts2/run_clone_indextts2_v2.sh
python cloneData/mimo/test_clone_mimo_v2.py
```

先附加 `--dry-run` 可检查三角色输入、模型目录和输出规划。输出已存在时脚本默认拒绝覆盖；只有确认是在重做本轮 V2 音频时才附加 `--overwrite`。通用本地模型按角色启动独立子进程并随退出释放显存；IndexTTS2 串行复用一次模型加载，并在 `finally` 中删除模型对象和清理 CUDA 缓存。

## Task 4 V3 用例

V3 脚本名称统一带 `_v3`，使用 `testData/mimo_<人物>_v3.wav` 三段参考音频，角色为旁白、小公主和三皇子。所有输出写入 `cloneData/audio_v3/`，命名为 `<模型名>_<人物>.wav`，不会覆盖上一轮音频。
