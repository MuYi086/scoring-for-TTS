...
以前的V4版本评价报告可能存在脏数据导致报告不准确，所以我才需要你重新运行一次评测

然后统计上面三份评价报告，按照真实世界小说生产有声音频的工作流分配权重，将三份文档按照统一标准体系权重计算分数综合后给出可以按照分数排名的最终`longAudioTest/评测结果/小说转有声TTS_V4综合评价报告.md`，也输出到`longAudioTest/评测结果`目录