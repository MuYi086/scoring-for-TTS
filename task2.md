...
然后在cloneData中创建对应模型的目录，然后在该目录下创建该模型的测试脚本，分别测试克隆以下三个音频，应该会用到"参考音频,text"，其他的参数我也给了，具体看模型是否需要。然后该脚本执行后的输出目录为cloneData目录,音频命名分别按照对应模型+人物命名。脚本要求模型执行完成后需要从显存中移除，避免占用显存。
集中克隆测试:
1. * 人物: 旁白
   * 音色: 男声，青年，清亮微磁。
...
全部模型克隆完成后
1. 使用tts-bench对每个模型克隆的音频和原始音频做对比，输出一份tts-bench评价报告。
2. 使用WavLM对每个模型克隆的音频和原始音频做对比，输出一份wavLM评估报告