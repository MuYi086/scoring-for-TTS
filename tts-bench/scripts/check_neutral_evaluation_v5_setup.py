#!/usr/bin/env python3
"""在加载评价模型前检查 Task 6 V5 环境、权重、长音频和冻结哈希。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_ROOT / "tts-bench" / "config" / "neutral-evaluation-v5.json",
    )
    parser.add_argument(
        "--assets",
        type=Path,
...
def check_environment(report: CheckReport, config: dict[str, Any]) -> tuple[Path, Path] | None:
    ...
    if not mirror_root_value:
        report.errors.append("必须设置 HF_MIRROR_ROOT，V5 评测不允许隐式联网下载")
        return None
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            report.errors.append("V5 配置要求 CUDA，但 torch.cuda.is_available() 为 False")
...
def check_inputs(report: CheckReport, config: dict[str, Any]) -> None:
    try:
        ...
        report.errors.append(f"V5 输入检查失败：{exc}")
        return
    ...
    report.passed.append(
        f"V5 输入完整：{len(references)} 条角色参考音频、{len(syntheses)} 条模型长音频"
    )
    report.passed.append(
