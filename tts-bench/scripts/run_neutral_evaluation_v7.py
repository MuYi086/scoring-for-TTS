...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--resume", action="store_true", help="仅续跑同一次未完成的 V7 评测。")
    parser.add_argument("--strict", action="store_true", help="所选指标存在缺失或错误时返回非零状态。")
    return parser.parse_args()


def main() -> int:
    return run(parse_args(), expected_schema_version="7.0", version_label="V7")
...
if __name__ == "__main__":
    try:
        ...
        print(f"V7 公共评测失败：{error}", file=sys.stderr)
