...
DEFAULT_OUTPUT = PROJECT_ROOT / "longAudioTestV6" / "评测结果" / "task7-v6-raw"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--reports-dir",
        type=Path,
        default=PROJECT_ROOT / "longAudioTestV6" / "评测结果",
        help="单模型六后端完成后写入独立评价报告的目录。",
    )
    parser.add_argument("--resume", action="store_true", help="续跑同一次未完成的 V6 评测。")
...
if __name__ == "__main__":
    try:
        ...
        print(f"V6 长音频中立评测失败：{error}", file=sys.stderr)
