...
REPORT_FILENAMES = {
    "cer": "SenseVoice_CER&Whisper-large-v3-turbo_CER_V6评价报告.md",
    "delivery": "音频交付与文本一致性_V6自动检查报告.md",
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--results-dir", type=Path, required=True, help="已完成的公共 V6 原始结果目录。")
    parser.add_argument("--reports-dir", type=Path, default=DEFAULT_REPORTS)
    return parser.parse_args()


def build_reports(results_dir: Path, results_link: str | None = None) -> dict[str, str]:
    """以 V6 冻结角色和版本标识渲染两份公共报告。"""
...
if __name__ == "__main__":
    try:
        ...
        print(f"V6 公共报告生成失败：{error}", file=sys.stderr)
