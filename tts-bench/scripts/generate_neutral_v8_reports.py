#!/usr/bin/env python3
"""从 Task 9 V8 原始结果生成单模型、三维度和综合中文评价报告。"""
...
DEFAULT_RESULTS = PROJECT_ROOT / "longAudioTestV8" / "评测结果" / "task9-v8-raw"
DEFAULT_REPORTS = PROJECT_ROOT / "longAudioTestV8" / "评测结果"
REPORT_FILENAMES = {
    "cer": "SenseVoice_CER&Whisper_CER_V8评价报告.md",
    "sim": "WavLM_SIM&SpeechBrain_ECAPA_SIM_V8评价报告.md",
    "quality": "UTMOSv2&NISQA_V8评价报告.md",
    # task9.md 明确指定该文件名；报告正文与数据版本仍为 V8。
    "comprehensive": "小说转有声TTS_V5综合评价报告.md",
}
ROLE_ORDER = {"我": 0, "旁白": 1, "姐姐": 2, "神秘声音": 3}


def configure_shared_renderer() -> None:
    """为复用的长音频报告渲染器注入 V8 冻结常量。"""

    shared.REPORT_VERSION = "V8"
    shared.SCHEMA_VERSION = "8.0"
    shared.SOURCE_DISPLAY_PATH = "longAudioTestV8/ai_deal.json"
    shared.CONFIG_DISPLAY_PATH = "tts-bench/config/neutral-evaluation-v8.json"
...
if __name__ == "__main__":
    try:
        ...
        print(f"V8 报告生成失败：{error}", file=sys.stderr)
