...
def evaluation_version(config: dict[str, Any]) -> str:
    ...
    if schema_version not in {"4.0", "5.0", "6.0", "7.0", "8.0", "9.0"}:
        raise ValueError("长音频评测仅支持 schema_version=4.0、5.0、6.0、7.0、8.0 或 9.0")
    return f"V{schema_version.split('.', maxsplit=1)[0]}"


def validate_config(config: dict[str, Any]) -> None:
    """校验 V4–V9 长音频配置中会影响统计对象的冻结约束。"""
