#!/usr/bin/env python3
"""在加载评价模型前检查 Task 9 V8 环境、权重、长音频和冻结哈希。"""
...
def check_environment(report: CheckReport, config: dict[str, Any]) -> tuple[Path, Path] | None:
    ...
    if not mirror_root_value:
        report.errors.append("必须设置 HF_MIRROR_ROOT，V8 评测不允许隐式联网下载")
        return None
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            report.errors.append("V8 配置要求 CUDA，但 torch.cuda.is_available() 为 False")
...
def check_inputs(report: CheckReport, config: dict[str, Any]) -> None:
    try:
        ...
        report.errors.append(f"V8 输入检查失败：{exc}")
        return
    ...
    report.passed.append(
        f"V8 输入完整：{len(references)} 条角色参考音频、{len(syntheses)} 条模型长音频"
    )
    report.passed.append(
